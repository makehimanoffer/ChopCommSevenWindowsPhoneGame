...
namespace ChopComm7.Scenes
{
    class Lose : Scene
    {
        ...
        {
            background = game.Content.Load<Texture2D>("Sprites\\Lose");
        ...
        public override void Draw(Microsoft.Xna.Framework.GameTime gameTime)
        {
            ...
            if (display_medals)
            {
                spriteBatch.DrawString(spritefont, "MEDALS UNLOCKED", new Vector2(300, 60), Color.White);
                for (int i = 0; i < 4; i++)
                {
                    if (medalstrue[i] == true)
                    {
                        spriteBatch.Draw(RankManager.medals1[i], new Rectangle(100, 100, 0, 100), Color.White);
                    }
                }
                for (int i = 4; i < 8; i++)
                {
            ...
            //spriteBatch.DrawString(spritefont, "Credits", new Vector2(20, 20), Color.White);
          
            spriteBatch.DrawString(spritefont, "Return to Main", new Vector2(200, 300), Color.White,0f,Vector2.Zero,1f,SpriteEffects.None,0f);
            spriteBatch.End();
        }
    }
}